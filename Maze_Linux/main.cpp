...
int mazePos[2] = {static_cast<int>(0.0), static_cast<int>(0.0)};
...
public:
    ...
    void load()
    {
        ...
        while(true) {
            ...
            readYWalls(line, lineSize, &(bYWall[y][0]));
            // get x walls
            if(fgets(line,MAX_LINE_SIZE,file)==nullptr) break;
            lineSize = (int)strlen(line) - 1;
            x= readXWalls(line, lineSize, &(bXWall[y][0]));
...
void message()
{
    ...
    glRasterPos2f(33, 300);
    std::string str = "Congratulations, you won!";
    for (char i : str)
...
void success()
{
//    MessageBox(nullptr, "Congratulations, you won!", "Exit", 0);
    glutInitDisplayMode(GLUT_RGB | GLUT_DOUBLE);
    glutInitWindowSize(300, 48);
    glutInitWindowPosition(500, 350);
    glutCreateWindow("Exit from maze");
...
void init()
{
    ...
    mazePos[0] = maze.initX;
    mazePos[1] = maze.initY;
...
void turn(MazeTurnDirection turnDir)
{
    ...
    for(int i = 0; i < TURN_NUMBER_OF_STEP; i++)
    {
        ...
        glTranslatef(position[0], position[1], position[2]);
        glRotatef((float)turnDir * TURN_ANGLE / (float)TURN_NUMBER_OF_STEP, 0.0, 1.0f, 0.0);
        glTranslatef(-position[0], -position[1], -position[2]);

        // update the direction
        float saveDir = direction[0];
        direction[0] = (float)(cos(-((float)turnDir * TURN_ANGLE / (float)TURN_NUMBER_OF_STEP) / 180.0 * PI) * direction[0]
                       + sin(-((float)turnDir * TURN_ANGLE / (float)TURN_NUMBER_OF_STEP) / 180.0 * PI) * direction[2]);
        direction[2] = (float)(-sin(-((float)turnDir * TURN_ANGLE / (float)TURN_NUMBER_OF_STEP) / 180.0 * PI) * saveDir
                       + cos(-((float)turnDir * TURN_ANGLE / (float)TURN_NUMBER_OF_STEP) / 180.0 * PI) * direction[2]);
